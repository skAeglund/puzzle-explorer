...
(function (root) {
  ...
  var NAME_MAX = 80;
  var LABEL_MAX = 120;
  var MAX_ITEMS_PER_REPERTOIRE = 2000;  // soft sanity cap; LS would die long before this.
                                        // Raised from 500 when Lichess study import landed —
                                        // a deep study with sidelines can yield 500+ unique
                                        // posKeys in one import. 2000 keeps headroom for
                                        // multi-study unions without approaching LS quota.
  ...
  var api = {
    ...
    // Constants — exposed so UI code (e.g. study-import preview that
    // shows "X room left in repertoire") can stay in sync without
    // hardcoding a magic number that drifts from the lib.
    MAX_ITEMS_PER_REPERTOIRE: MAX_ITEMS_PER_REPERTOIRE
  };
