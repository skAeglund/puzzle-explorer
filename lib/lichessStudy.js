...
(function (root) {
  ...
  function walkChapter(chapter, opts) {
    ...
    // Stack of { fen, ply, sanLine, prevFen } snapshots, one per open '('
    // we're currently inside. On '(' we save the state we'll restore at ')'
    // and rewind chess to BEFORE the most recent move (the variation is an
    // alternative to that move). We can't use chess.undo() for the rewind
    // because chess.load() (used inside ')' handling) resets the engine's
    // move history — so undo would be a no-op after returning from a sister
    // variation. Instead we track prevFen explicitly.
    //
