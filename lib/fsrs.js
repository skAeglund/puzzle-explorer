...
(function (root) {
  ...
  // DESIRED_RETENTION drives the spacing curve — FSRS schedules the next
  // review when retrievability decays to this value. 0.9 is the FSRS-5
  // upstream default, but in practice it produces an aggressive Easy
  // ramp (Again→7d→47d→291d→cap), graduating cards in fewer reviews
  // than feels useful for a chess-puzzle context. 0.93 calms it to
  // 4d→21d→99d→365d — still self-retiring, but the user sees the
  // puzzle 2-3 more times along the way. Tunable; lowering toward
  // 0.9 reverts to upstream behavior, raising toward 0.95 makes
  // every card resurface more often.
  var DESIRED_RETENTION = 0.93;
  ...
  // Per project plan: wrong attempt → again, hint used → hard.
  // Clean solves (no wrong, no hint) are graded by average per-move
  // thinking time when avgMoveTimeMs is supplied — slow → hard, medium →
  // good, fast → easy. Without timing data, falls back to easy (preserves
  // pre-timing behavior; tests + legacy callers unaffected).
  //
  // Thresholds (initial heuristic, easy to tune later). Boundaries land
  // in the lower bucket because the comparisons are strict `>`:
  //   avg ≤ 10s/move        → easy (4)
  //   avg 10s < x ≤ 60s     → good (3)
  //   avg > 60s/move        → hard (2)
  function gradeForAttempt(opts) {
    if (opts && opts.wrong) return FSRS_GRADE.again;
    if (opts && opts.hintUsed) return FSRS_GRADE.hard;
    var avg = opts && opts.avgMoveTimeMs;
    if (typeof avg === 'number' && Number.isFinite(avg)) {
      if (avg > 60000) return FSRS_GRADE.hard;
      if (avg > 10000) return FSRS_GRADE.good;
    }
