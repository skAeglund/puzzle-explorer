...
(function (root) {
  ...
  var _pending = null;     // { id, version } downloaded-but-not-yet-applied bundle
  ...
  function checkAndStage(opts) {
    ...
    return Promise.all([loadLocalVersion(), fetchManifest(manifestUrl)])
      .then(function (res) {
        ...
        // Already downloaded this (or newer) in this session — don't re-fetch.
        if (_pending && _pending.version >= decision.version) {
          return { ok: true, updated: true, staged: true, version: _pending.version, bundleId: _pending.id };
        }
        return Promise.resolve()
          ...
          .then(function (bundle) {
            ...
            // Download ONLY — do NOT set() here. A set-but-not-reloaded bundle
            // sits in a "pending validation" state that Capgo can revert across
            // launches; combined with our re-check re-staging it, that caused
            // version ping-pong. Instead we stash it and switch only on an
            // explicit applyNow() (the "Restart now" button), which set()s +
            // reload()s together so the new bundle loads immediately and
            // confirms within Capgo's timeout. If the user never taps Restart,
            // nothing is swapped and there's no bundle in limbo.
            _pending = { id: bundle.id, version: decision.version };
            return { ok: true, updated: true, staged: true, version: decision.version, bundleId: bundle.id };
          })
          .catch(function (e) {
            return { ok: false, updated: false, reason: 'download-failed',
  ...
  // Apply a downloaded update: activate the stashed bundle, then reload into
  // it. set()+reload() together is Capgo's clean apply path — the new bundle
  // loads immediately and confirms (notifyReady at end of init) within the
  // rollback timeout, so there's no pending-validation limbo. Used by the
  // "Restart now" button. No-op/failure-safe when unavailable or nothing
  // pending (then it's just a reload).
  function applyNow() {
    var p = getPlugin();
    if (!p || typeof p.reload !== 'function') return Promise.resolve(false);
    return Promise.resolve()
      .then(function () {
        if (_pending && _pending.id && typeof p.set === 'function') {
          return p.set({ id: _pending.id });
        }
      })
  ...
  var api = {
    ...
    // test seams
    _setPlugin: function (p) { pluginImpl = p || null; _pending = null; },
