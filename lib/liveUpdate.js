...
(function (root) {
  ...
  // Apply a staged bundle immediately by reloading the WebView into it. Only
  // meaningful right after checkAndStage() reported staged:true. Used by the
  // "Restart now" button; otherwise the staged bundle applies on next launch.
  // No-op/failure-safe when unavailable.
  function applyNow() {
    var p = getPlugin();
    if (!p || typeof p.reload !== 'function') return Promise.resolve(false);
    return Promise.resolve().then(function () { return p.reload(); })
      .then(function () { return true; })
      .catch(function () { return false; });
  }
