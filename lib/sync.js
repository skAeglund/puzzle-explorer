 ...
 *   - keepalive: true on the PATCH ONLY on the pagehide/flush path (flushNow)
 *     and only when body < 64KB (browser cap). The regular debounced/retry
 *     sync never sets it: some Android System WebViews (the Capacitor native
 *     build) reject a keepalive PATCH, which failed every normal sync while
 *     reads kept working. Above the cap, browsers reject keepalive and fall
 *     back to a fetch
...
(function (root) {
  ...
  // Personal-tool defaults: a shared private gist owned by skAeglund.
  // Anyone running this build can paste their own PAT and connect without
  // juggling a gist ID. The PAT is the actual secret — the gist ID is
  // just a long opaque hex string that's useless without the PAT, so
  // committing it is safe. Users who want their OWN per-user gist can
  // clear the gist ID field; that hits the createGist path and stamps
  // their typed username into a brand new private gist.
  // The username default must match the gist body's stored `username`
  // field — the cross-device sanity check rejects mismatches. If one
  // changes, the other must change too.
  var DEFAULT_USERNAME   = 'anders';
  ...
  // Lifted from MistakeLab's mergeGistData. Strategy:
  //   - Per-id: newer lastSeen wins for the entire entry. For positions
  //     this treats the {completed, lastSeen, srs} triple as one
  //     timestamped record; for repertoires it treats {name, items, ...}
  //     the same way.
  //   - Live-vs-live winner uses {...remote, ...winner} to keep any
  //     forward-compat fields the remote side knows about that the local
  //     side doesn't.
  //   - Tombstones (entries with deleted:true) are absolute — winner takes
  //     the entry as-is, no spread. Used by repertoires for user-initiated
  //     deletes, and by positions for orphan puzzles that Progress.forget()
  //     could no longer load from any body shard. Without the no-spread
  //     posture: (a) a winning tombstone would drag stale name/items fields
  //     from the loser, and (b) a winning LIVE entry would inherit
  //     deleted:true from a losing tombstone via the remote→winner spread,
  //     which would silently re-tombstone the resurrection.
  //   - Disjoint ids: union (both sides keep their own).
  ...
  function merge(local, remote) {
    if (!local && !remote) return { positions: {}, repertoires: {}, version: BODY_VERSION };
    if (!local)  return JSON.parse(JSON.stringify(remote));
    if (!remote) return JSON.parse(JSON.stringify(local));
    // Start from a deep clone of remote; carry over local's branches on top.
    var merged = JSON.parse(JSON.stringify(remote));
    if (typeof merged.version !== 'number') merged.version = BODY_VERSION;
    _mergeKeyedBranch(local, merged, 'positions');
    _mergeKeyedBranch(local, merged, 'repertoires');

    // Streak meta (issue #9). The naive deep-clone-of-remote already
    // carries remote.meta forward, but local.meta would otherwise be
    // dropped — meaning a device with an active streak would lose it the
    // moment it pulls a remote that hasn't been updated as recently. Pick
    // whichever side's meta has the most recent lastReviewDay (tiebreak
    // on longestStreak) for the time-sensitive fields lastReviewDay and
    // currentStreak. longestStreak is max-merged across BOTH sides so a
    // historical personal-best survives a sync from a less-active device.
    // YYYY-MM-DD strings sort lexically, which is correct for date order.
    var localMeta  = (local.meta && typeof local.meta === 'object' && !Array.isArray(local.meta)) ? local.meta : null;
    var remoteMeta = (merged.meta && typeof merged.meta === 'object' && !Array.isArray(merged.meta)) ? merged.meta : null;
    if (localMeta && remoteMeta) {
      var ld = (typeof localMeta.lastReviewDay  === 'string') ? localMeta.lastReviewDay  : '';
      var rd = (typeof remoteMeta.lastReviewDay === 'string') ? remoteMeta.lastReviewDay : '';
      var lLong = (typeof localMeta.longestStreak  === 'number' && isFinite(localMeta.longestStreak))  ? (localMeta.longestStreak  | 0) : 0;
      var rLong = (typeof remoteMeta.longestStreak === 'number' && isFinite(remoteMeta.longestStreak)) ? (remoteMeta.longestStreak | 0) : 0;
      var bestLong = (lLong > rLong) ? lLong : rLong;
      if (ld > rd || (ld === rd && lLong > rLong)) {
        merged.meta = JSON.parse(JSON.stringify(localMeta));
      }
      // else: keep merged.meta (already a clone of remote.meta).
      // Either way, override longestStreak with the cross-side max — a
      // user who hit 30 days on device A then took a break and is now
      // building a new streak on device B should still see "best 30".
      merged.meta.longestStreak = bestLong;
    } else if (localMeta) {
      merged.meta = JSON.parse(JSON.stringify(localMeta));
    }
    return merged;
  ...
  // `first` (first-attempt theme outcome) is IMMUTABLE — set once on a
  // puzzle's genuine first attempt and never changed. The field-level spread
  // below already preserves it when only one side has it (the other side
  // simply lacks the key). The one case the spread gets wrong is concurrent
  // offline first attempts of the SAME fresh puzzle on two devices: both set
  // `first` independently, possibly with different verdicts, and the spread
  // would keep the lastSeen-winner's — not necessarily the chronological
  // first. Since `first` is immutable, the chronologically-earliest attempt
  // is the true one: prefer the earlier `at`. Returns the winning `first`
  // object, or undefined if neither side has a valid one.
  function _validFirst(f) {
    return !!(f && typeof f === 'object' && !Array.isArray(f) && typeof f.pass === 'boolean');
  }
  function _firstAt(f) {
    if (!f || typeof f.at !== 'string') return Infinity;  // no timestamp → can't claim "earliest"
    var t = new Date(f.at).getTime();
    return isFinite(t) ? t : Infinity;
  }
  function _reconcileFirst(localEntry, remoteEntry) {
    var lf = localEntry && localEntry.first;
    var rf = remoteEntry && remoteEntry.first;
    var lOk = _validFirst(lf), rOk = _validFirst(rf);
    if (!lOk && !rOk) return undefined;
    if (lOk && !rOk) return lf;
    if (!lOk && rOk) return rf;
    return (_firstAt(lf) <= _firstAt(rf)) ? lf : rf;  // earliest wins; ties → local (deterministic)
  }
  // `userThemes` (manual tags) are MUTABLE, so unlike `first` the LATEST edit
  // wins — newest `userThemesAt`. The whole array is the unit (last-write-wins),
  // which correctly propagates removals (an older non-empty array loses to a
  // newer empty one). Returns { userThemes, userThemesAt } or undefined when
  // neither side has tags. Acts independently of the lastSeen winner because
  // tagging doesn't bump lastSeen.
  function _userThemesAt(e) {
    if (!e || typeof e.userThemesAt !== 'string') return -Infinity;  // no edit loses to any real edit
    var t = new Date(e.userThemesAt).getTime();
    return isFinite(t) ? t : -Infinity;
  }
  function _reconcileUserThemes(localEntry, remoteEntry) {
    var lHas = !!(localEntry && Array.isArray(localEntry.userThemes));
    var rHas = !!(remoteEntry && Array.isArray(remoteEntry.userThemes));
    if (!lHas && !rHas) return undefined;
    if (lHas && !rHas) return { userThemes: localEntry.userThemes, userThemesAt: localEntry.userThemesAt };
    if (!lHas && rHas) return { userThemes: remoteEntry.userThemes, userThemesAt: remoteEntry.userThemesAt };
    return (_userThemesAt(localEntry) >= _userThemesAt(remoteEntry))  // newest wins; ties → local
      ? { userThemes: localEntry.userThemes, userThemesAt: localEntry.userThemesAt }
      : { userThemes: remoteEntry.userThemes, userThemesAt: remoteEntry.userThemesAt };
  }
  // Per-id latest-wins merge of one branch (positions, repertoires, …)
  // from `local` onto `mergedRoot[key]` (which already holds remote's
  // copy from the deep-clone seed in merge()). See merge()'s comment for
  // the tombstone rationale.
  function _mergeKeyedBranch(local, mergedRoot, key) {
    if (!mergedRoot[key] || typeof mergedRoot[key] !== 'object' || Array.isArray(mergedRoot[key])) {
      mergedRoot[key] = {};
    }
    if (!local[key] || typeof local[key] !== 'object' || Array.isArray(local[key])) return;
    for (var id in local[key]) {
      if (!Object.prototype.hasOwnProperty.call(local[key], id)) continue;
      var localEntry = local[key][id];
      if (!localEntry || typeof localEntry !== 'object') continue;
      var remoteEntry = mergedRoot[key][id];
      if (!remoteEntry) { mergedRoot[key][id] = JSON.parse(JSON.stringify(localEntry)); continue; }
      ...
      if (localEntry.deleted === true || remoteEntry.deleted === true) {
        // Tombstone-touching merges bypass spread — see merge()'s comment.
        mergedRoot[key][id] = JSON.parse(JSON.stringify(winner));
        continue;
      }
      var spread = {};
      ...
      // `first` is immutable — reconcile independently of the lastSeen winner
      // so a concurrent offline first attempt isn't lost. No-ops for branches
      // whose entries never carry `first` (e.g. repertoires).
      var reconciledFirst = _reconcileFirst(localEntry, remoteEntry);
      if (reconciledFirst !== undefined) spread.first = JSON.parse(JSON.stringify(reconciledFirst));
      // Manual tags reconcile independently (newest-wins). No-ops for branches
      // whose entries never carry them.
      var reconciledUT = _reconcileUserThemes(localEntry, remoteEntry);
      if (reconciledUT !== undefined) {
        spread.userThemes = JSON.parse(JSON.stringify(reconciledUT.userThemes));
        if (typeof reconciledUT.userThemesAt === 'string') spread.userThemesAt = reconciledUT.userThemesAt;
      }
      mergedRoot[key][id] = spread;
    }
  }
  // Used by loadFromGist to decide whether a follow-up sync is needed.
  // Returns true iff at least one local entry across the synced branches
  // (positions, repertoires) is either (a) strictly newer than its remote
  // counterpart, or (b) absent from remote.
  function _localHasNewerOrUnique(local, remote) {
    if (!local) return false;
    return _branchHasNewerOrUnique(local, remote, 'positions') ||
           _branchHasNewerOrUnique(local, remote, 'repertoires');
  }
  function _branchHasNewerOrUnique(local, remote, key) {
    var lBranch = local && local[key];
    if (!lBranch || typeof lBranch !== 'object' || Array.isArray(lBranch)) return false;
    var rBranch = (remote && remote[key]) ? remote[key] : null;
    for (var id in lBranch) {
      if (!Object.prototype.hasOwnProperty.call(lBranch, id)) continue;
      var le = lBranch[id];
      if (!le || typeof le !== 'object') continue;
      if (!rBranch || !rBranch[id]) return true;          // local-only entry
      if (_entryTime(le) > _entryTime(rBranch[id])) return true;
    }
    return false;
  }
  ...
  function gistWrite(data, allowKeepalive) {
    ...
    if (allowKeepalive && body.length < KEEPALIVE_MAX_BYTES) {
      // keepalive:true lets the request survive page unload — needed ONLY on
      // the pagehide/flush path (flushNow). It is deliberately NOT used on the
      // regular debounced/retry sync, because some Android System WebViews
      // (i.e. the Capacitor native build) reject a keepalive PATCH outright,
      // failing every normal sync with a network error while reads still work.
      // Above the 64KB cap browsers reject keepalive regardless, so drop it.
      opts.keepalive = true;
    }
  ...
  function _syncToGist(fromFlush) {
    ...
    return gistRead().then(function (remote) {
      ...
      if (typeof merged.version !== 'number') merged.version = BODY_VERSION;
      // keepalive only when this is the pagehide/flush call (see gistWrite).
      return gistWrite(merged, fromFlush === true).then(function () {
