...
(function (root) {
  ...
  // clean no-hint solve is true; any wrong move or hint is false). Returns the
  // entry if it WROTE a new `first`, or null if it did not write (bad id, or
  // the entry already carries a valid `first` — the genuine first verdict is
  ...
  function markFirstAttempt(puzzleId, pass, themes, now) {
    ...
    var nowDate = (now instanceof Date && Number.isFinite(now.getTime()))
      ? now
      : new Date();
    entry.first = { pass: !!pass, themes: _cleanThemes(themes), at: nowDate.toISOString() };
    data.positions[puzzleId] = entry;
    save(data);
    return entry;
  }
  ...
  // per-theme attempt/fail counts. One puzzle contributes to each of its
  // themes. Only entries with a valid `first` count (forward-only; pre-feature
  // puzzles have none). Tombstones are skipped. Returns:
  //   { totalPuzzles, themes: { <theme>: { attempts, fails } } }
  // successRate(t) = (attempts - fails) / attempts. Pure read over load().
  function themeStats() {
    var data = load();
    ...
    for (var id in positions) {
      ...
      out.totalPuzzles++;
      var ths = Array.isArray(e.first.themes) ? e.first.themes : [];
      for (var j = 0; j < ths.length; j++) {
        var t = ths[j];
        if (typeof t !== 'string' || !t) continue;
        var b = out.themes[t] || (out.themes[t] = { attempts: 0, fails: 0 });
        b.attempts++;
        if (!e.first.pass) b.fails++;
      }
    }
    return out;
  }
