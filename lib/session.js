...
(function (root) {
  ...
  // Filter [id, rating, color, ply] tuples by inclusive ply bounds. Same
  // contract as filterByRating: missing/non-numeric ply (m[3]) passes
  // through unfiltered — older shards may not carry the ply field, and
  // we don't silently drop those puzzles. Reads m[3] specifically; the
  // tuple position is the contract with build-index.js.
  //
  // The ply field encodes the source-game ply at which the indexed
  // position occurs (1-indexed; the position AFTER the move at that ply).
  // Filtering "max ply 8" gives positions reachable in the first 8
  // halfmoves of the source game — useful for "opening only" sessions.
  function filterByPly(matches, plyMin, plyMax) {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      var ply = m[3];
      if (typeof ply === 'number' && isFinite(ply)) {
        if (ply < min || ply > max) continue;
      }
      out.push(m);
    }
    return out;
   ...
   *
   * Within-round ordering: a `target`-sized random sample is drawn from
   * the in-range pool (uniform via Fisher-Yates), then sorted by rating
   * ascending so the user warms up before the round's hardest puzzles.
   * The sort is stable; ties keep the random order from the sample.
   * Tuples without a numeric rating sort to the end of the round so they
   * don't disrupt the warm-up curve.
   *
  ...
  function createTraining(opts) {
    ...
    for (var i = 0; i < roundsSpec.length; i++) {
      ...
      for (var j = 0; j < inRange.length; j++) {
        ...
        var pidRating = (typeof m[1] === 'number' && isFinite(m[1])) ? m[1] : null;
        unsolved.push({ id: pid, rating: pidRating });
      }
      ...
      // the shuffled order (tie-breaks stay random). Rating-less entries
      // sort to the end; two rating-less entries compare equal so their
      // shuffle-order is preserved (Infinity - Infinity is NaN, which makes
      // the sort implementation-defined — guard explicitly).
