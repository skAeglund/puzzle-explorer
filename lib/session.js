...
(function (root) {
  ...
  // Filter [id, rating, color, ply, startPly, themeCodes] tuples by theme.
  // `themeCodes` is the user's SELECTED set — an array or Set of integer
  // theme codes (see lib/themes.js). A tuple matches if its m[5] shares ANY
  // selected code (OR / union semantics — "forks OR pins"). Empty / null
  // selection ⇒ no-op pass-through (return a copy), same shape as
  // filterByPly's wide-open fast path.
  //
  // Back-compat, three cases, deliberately distinguished:
  //   - m[5] === undefined  (legacy shard built before themes were added,
  //                          or an index not yet republished with m[5]) →
  //                          PASS THROUGH even under an active selection.
  //                          Same posture as filterByPly on missing m[4]
  //                          and filterByPerspective on missing m[2]. This
  //                          is what makes the whole feature ship inert:
  //                          before the data carries m[5], every entry
  //                          passes and the filter no-ops.
  //   - m[5] === [] (present, empty) → puzzle has NO curated theme → DROP
  //                          under an active selection (it matches nothing).
  //   - m[5] === [codes…]    → keep iff intersection with selection.
  // The undefined-vs-[] distinction is the crux; don't collapse them.
  function toThemeCodeSet(themeCodes) {
    if (!themeCodes) return null;
    if (themeCodes instanceof Object && typeof themeCodes.has === 'function' && typeof themeCodes.size === 'number') {
      return themeCodes.size > 0 ? themeCodes : null;   // already a Set
    }
    if (!Array.isArray(themeCodes) || themeCodes.length === 0) return null;
    var set = {};
    var any = false;
    for (var i = 0; i < themeCodes.length; i++) {
      var code = themeCodes[i];
      if (typeof code === 'number' && isFinite(code)) { set[code] = true; any = true; }
    }
    if (!any) return null;
    // Wrap the plain object in a tiny has()-shaped facade so the hot loop
    // below is uniform whether we were handed a Set or an array.
    return { has: function (x) { return set[x] === true; }, size: 1 };
  }
  function filterByTheme(matches, themeCodes) {
    var sel = toThemeCodeSet(themeCodes);
    if (!sel) return matches.slice();                 // no-op: nothing selected
    var out = [];
    for (var i = 0; i < matches.length; i++) {
      var m = matches[i];
      if (!m || m.length < 1) continue;
      var codes = m[5];
      if (codes === undefined || codes === null) { out.push(m); continue; } // legacy pass-through
      if (!Array.isArray(codes)) { out.push(m); continue; }                  // defensive: odd shape
      var hit = false;
      for (var j = 0; j < codes.length; j++) {
        if (sel.has(codes[j])) { hit = true; break; }
      }
      if (hit) out.push(m);
    }
    return out;
  ...
  function create(opts) {
    ...
    // Theme filter (OR/union over selected codes). No-op when nothing is
    // selected. Applied between ply and rating — order is immaterial to
    // correctness (all three are independent inclusive filters).
    var afterTheme = filterByTheme(afterPly, opts.themeCodes);
    var inRange = filterByRating(afterTheme, opts.ratingMin, opts.ratingMax);
  ...
  function createTraining(opts) {
    ...
    // Theme filter applied once at the top too, so every round draws from
    // the same theme-bounded pool. No-op when nothing is selected.
    matches = filterByTheme(matches, opts.themeCodes);
