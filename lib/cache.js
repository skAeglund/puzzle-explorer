...
(function (root) {
  ...
  function dbGetAllKeys(storeName) {
    return openDb().then(function (db) {
      ...
      try {
        ...
        return promisifyRequest(store.getAllKeys()).then(function (v) {
          return v || [];
        });
      } catch (e) { return []; }
    }).catch(function () { return []; });
  ...
  // dbPutMany — put several records in ONE transaction. Used by the offline
  // body prefetch, which writes hundreds of bodies at once; a per-record
  // transaction round-trip would be needlessly slow. Resolves when the tx
  // settles (complete/error/abort), never rejects.
  function dbPutMany(storeName, values) {
    ...
    return openDb().then(function (db) {
      ...
      try {
        var tx = db.transaction(storeName, 'readwrite');
        var store = tx.objectStore(storeName);
        for (var i = 0; i < values.length; i++) store.put(values[i]);
        return new Promise(function (resolve) {
          tx.oncomplete = function () { resolve(); };
          tx.onerror    = function () { resolve(); };
          tx.onabort    = function () { resolve(); };
  ...
  /**
   * putOfflineBodies(records) → Promise<void>
   * records: [{ id, body }]. Stored as { key:id, body, storedAt }. Batched into
   * a single transaction. Idempotent — re-storing the same id overwrites.
   */
  ...
  function putOfflineManifest(repId, manifest) {
    if (typeof repId !== 'string' || !repId) return Promise.resolve();
    var value = { key: repId };
    ...
    return dbPut(OFFLINE_MANIFEST_STORE, value);
  }
